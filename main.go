...
func main() {
	...
	if os.Getenv("RELAX_BOTS_KEY") == "" || os.Getenv("RELAX_BOTS_PUBSUB") == "" ||
		...
		fmt.Printf("usage: RELAX_MUTEX_KEY=relax_mutex_key RELAX_BOTS_KEY=relax_bots_queue RELAX_BOTS_PUBSUB=relax_bots_pubsub RELAX_EVENTS_QUEUE=relax_events_queue REDIS_URL=redis://localhost:6379 relax\n")
		os.Exit(1)
	}
